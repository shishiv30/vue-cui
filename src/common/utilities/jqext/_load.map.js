    ...
    loadGMap: function() {
        ...
        if (mapLoaded === 2) {
            ...
            if (mapLoaded === 0) {
                ...
                var mapUrl = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBtspK44Jc-DabkyA6knFuQ6LSp_8VXZgc'
                $.each(config, function(key, value) {
                    mapUrl += ('&' + key + '=' + value)
                })
                $.preload({files: [mapUrl], type: 'js', callback: 'googlemapcallback'}).then(function() {
                    initialCustomMarker()
                    mapLoaded = 2
                    dfd.resolve(window.google && window.google.map)
